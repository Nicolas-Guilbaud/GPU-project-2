...
__global__ void shared_mem_kernel(
    ...
    const cudaTextureObject_t *Y_img,
    ...
    __shared__ uint8_t Y_img_ref[32*8*2*9];
    ...
    for(int k = 0; k < 3; k++){
        for(int l = 0; l < 3; l++){
            ...
            //pixel to pull
            uint8_t px = tex2DLayered<uint8_t>(*Y_img,x_shared,y_shared,ref_idx);
            
            Y_img_ref[3*threadIdx.x+k+SHARED_X_SIZE*(3*threadIdx.y+l + SHARED_Y_SIZE*threadIdx.z)]
                = px;
        }
    }
    ...
    //we have 512 threads that must pull 
    ...
    for(int cam_idx = 0; cam_idx < cam_vec_size; cam_idx++){
        ...
        for (int k = -window / 2; k <= window / 2; k++)
        {
            for (int l = -window / 2; l <= window / 2; l++)
            {
                ...
                // x+k is indexed at 0 in X axis of shared mem
                // same for y+k
                int idx_shared = (threadIdx.x + window/2 + k) 
                    + SHARED_X_SIZE * (threadIdx.y + window/2 + l + SHARED_Y_SIZE * threadIdx.z);
                uint8_t ref = Y_img_ref[idx_shared],
                    curr = tex2DLayered<uint8_t>(*Y_img,x_proj+l,y_proj+k,cam_idx);
...
std::vector<cv::Mat> shared_sweeping_plane(
    ...
    //data
    cudaArray *dev_Y_data;
    cudaMemcpy3DParms params = {0};

    //descriptors
    cudaResourceDesc dev_Y_res_desc;
    cudaTextureDesc dev_Y_tex_desc;
    memset(&dev_Y_tex_desc, 0, sizeof(cudaTextureDesc));

    cudaTextureObject_t dev_Y_img; //actual texture object
    ...
    //regroup Y images as a 2D texture that contains 4 layers 
    cudaMalloc3DArray(&dev_Y_data,&channel_desc,make_cudaExtent(width,height,cam_vec_size),cudaArrayLayered);
    ...
    for(int k = 0; k < cam_vec_size; k++){
        ...
        params.dstPos = make_cudaPos(0,0,k); //place img k on layer k
        params.srcPtr = make_cudaPitchedPtr(current.YUV[0].data,width*sizeof(uint8_t),width,height);
        params.dstArray = dev_Y_data;
        params.extent = make_cudaExtent(width,height,1); //copy only 1 layer !
        params.kind = cudaMemcpyHostToDevice;

        //actual copy
        CHK(cudaMemcpy3D(&params));
    }

    //resource descriptor: we store texture as a 3D cuda array
    dev_Y_res_desc.resType = cudaResourceTypeArray;
    dev_Y_res_desc.res.array.array = dev_Y_data;
    
    //texture descriptor
    //note: can be modified to potentialy optimize runtime at expense of cost precision
    dev_Y_tex_desc.addressMode[0] = cudaAddressModeBorder;
    dev_Y_tex_desc.addressMode[1] = cudaAddressModeBorder;
    dev_Y_tex_desc.filterMode = cudaFilterModePoint;
    dev_Y_tex_desc.readMode = cudaReadModeElementType;
    dev_Y_tex_desc.normalizedCoords = 0;

    // bind texture
    cudaCreateTextureObject(&dev_Y_img,&dev_Y_res_desc,&dev_Y_tex_desc,NULL);


    // run kernel
    shared_mem_kernel<<<N_blocks,max_threads_512>>>(
        ref_idx,        // ref
        &dev_Y_img,      // Y
        width,          // img size
        height,
...
Error:
    //Y
    cudaDestroyTextureObject(dev_Y_img);
    cudaFreeArray(dev_Y_data);
    //cost_mat
