#include "fast_convol_cpu_cast.cuh"

#define divup(x,y) (((x)+(y)-1)/(y))

inline __global__ void convolution_2D_naive_kernel(
    uint8_t *input,
    uint8_t *output,
    int width,
    int height,
...
__global__ void single_cam_fast_cost_fp32_kernel(
...
std::vector<cv::Mat> single_cam_fast_convol_cpu_cast(
    ...
    convolution_2D_naive_kernel<<<convol_blocks,max_threads_512>>>(
        convol_buffer,
        ref_Y_img,
        width,
        height,
