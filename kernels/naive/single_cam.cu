...
__global__ void single_cam_kernel(
    ...
    cost_mat[(x + width*(y + height*(zIdx)))] = cost;
}
...
std::vector<cv::Mat> single_cam(
    ...
    float* host_cost_mat = (float*) malloc(width*height*ZPlanes*sizeof(float)),
        *host_min_cost_mat = (float*) malloc(width*height*ZPlanes*sizeof(float));
    
    //set minimal value to 255
    memset(host_min_cost_mat,255.f,width*height*ZPlanes*sizeof(float));
    ...
    for(int cam_idx = 0; cam_idx < cam_vec_size; cam_idx++){
        ...
        //keep min value
        for(int i = 0; i < width*height*ZPlanes; i++){
            host_min_cost_mat[i] = fminf(host_cost_mat[i],host_min_cost_mat[i]);
        }
    }
    ...
    for(int zi = 0; zi < ZPlanes; zi++){
        ...
        float* host_cost_zi = host_min_cost_mat + zi*width*height;
